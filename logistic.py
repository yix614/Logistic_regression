...
def binary_train(X, y, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    for i in range(0, max_iterations):
        ...
        for n in range(0, len(X)):
            ...
            pred = sigmoid(b + wtx)
            error += (pred - y[n])
            gradient += (pred - y[n]) * np.array(X[n])
...
def binary_predict(X, w, b):
    ...
    wt = w.transpose()
    for i in range(0, N):
        wtx = np.matmul(wt, X[i])
        pred_prob = sigmoid(b + wtx)
        if pred_prob < pred_grid:
            preds[i] = 0
        else:
            preds[i] = 1
...
def multinomial_train(X, y, C, 
    ...
    for i in range(max_iterations):
        temp = np.exp((np.dot(X, w.T) + b - np.max(np.dot(X, w.T) + b)).T)
        pred = temp/temp.sum(axis=0)
        for j in range(N):
            pred[y[j]][j] = pred[y[j]][j] - 1
        gradient = np.dot(pred, X) / N
        error = np.sum(pred, axis=1) / N
        w = w - step_size * gradient    
        b = b - step_size * error
...
def OVR_train(X, y, C, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    for c in range(0, C):
        pred_class = np.vectorize(lambda x: 1 if x == c else 0)
        w[c], b[c] = binary_train(X, pred_class(y))
